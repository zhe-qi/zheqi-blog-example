...
<template>
  <div class="lg:px-[15%] px-[5%] w-full box-border overflow-x-hidden">
    <main class="pt-[4.5rem] w-full">
      <!-- 头上三个大板块 -->
      <div
        class="grid w-full grid-cols-1 grid-rows-3 lg:grid-cols-3 lg:grid-rows-1 lg:gap-4 lg:h-64"
      >
        <div
          class="grid grid-cols-2 grid-rows-2 gap-4 mb-5 overflow-hidden rounded dark:contrast-50 md:mb-0"
        >
          <div v-for="(item, index) in rdmImg.slice(2, 6)" :key="index" class="overflow-hidden rounded cursor-pointer w-50 h-30 ">
            <img
        ...
        <div
          class="w-full overflow-hidden rounded lg:h-full h-52"
        >
          <Swiper
          ...
          >
            <SwiperSlide v-for="slide in imgs" :key="slide.title">
              <img :src="slide.url" alt="slide.title" class="object-cover w-full h-full dark:brightness-75">
            </SwiperSlide>
          </Swiper>
        </div>
        <div>
      ...
      <section class="w-full pt-2 lg:pt-7">
        <div class="flex justify-between h-[1000px] overflow-y-hidden w-full">
          <div
            class="lg:w-[70%] overflow-hidden  w-full h-full border rounded"
          >
            <div
              class="flex justify-between w-full py-2 border-b dark:text-red-50"
            >
            ...
            <Transition>
              <div v-show="isFilter" class="w-full h-40 p-2">
                <div class="grid w-full grid-cols-3 grid-row-1 h-15">
                  ...
                  >
                  <button
                    class="ml-2 btn dark:hover:bg-zinc-600 btn-outline btn-info"
            ...
            <!-- 虚拟列表 -->
            <div
              class="w-full dark:text-green-50 h-[calc(100%-2.65rem)]"
            ...
            >
              <div v-bind="wrapperProps">
                ...
                >
                  <img
                    class="object-cover h-full w-28"
                    :src="item.data.img"
                    alt=""
                  >
                  <div class="ml-2">
          ...
          <aside class="w-[28%] lg:block hidden">
            ...
            >
              <h1
                class="text-xl font-bold text-center dark:text-yellow-50 rainbow-text"
              >
                {{ $t('关于本站') }}
              </h1>
            ...
            <div
              class="w-full border dark:text-yellow-50 h-[1000px] p-2"
            >
              <h1
                class="text-xl font-bold text-center rainbow-text dark:text-yellow-50"
              >
                {{ $t('开发工具') }}
              </h1>
              <ul class="w-full h-full">
                ...
                >
                  ...
                  >
                    <img
                      class="w-40 bg-center bg-cover h-30"
                      :src="item.img"
                      alt=""
                    >
